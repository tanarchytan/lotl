...
function checkIndexHealth(db: Database): void {
  ...
  if (needsEmbedding > 0) {
    ...
    if (pct >= 10) {
      process.stderr.write(warn(`Warning: ${needsEmbedding} documents (${pct}%) need embeddings. Run 'lotl embed' for better results.`) + "\n");
    } else {
      process.stderr.write(`${c.dim}Tip: ${needsEmbedding} documents need embeddings. Run 'lotl embed' to index them.${c.reset}\n`);
    }
  }

  // Check if most recent document update is older than 2 weeks
  if (daysStale !== null && daysStale >= 14) {
    process.stderr.write(`${c.dim}Tip: Index last updated ${daysStale} days ago. Run 'lotl update' to refresh.${c.reset}\n`);
  }
}
...
async function showStatus(): Promise<void> {
  ...
  const mcpCacheDir = process.env.XDG_CACHE_HOME
    ? resolve(process.env.XDG_CACHE_HOME, "lotl")
    : resolve(homedir(), ".cache", "lotl");
  ...
  if (needsEmbedding > 0) {
    console.log(`  ${c.yellow}Pending:  ${needsEmbedding} need embedding${c.reset} (run 'lotl embed')`);
  }
  ...
  if (collections.length > 0) {
    ...
    for (const col of collections) {
      ...
      console.log(`  ${c.cyan}${col.name}${c.reset} ${c.dim}(lotl://${col.name}/)${c.reset}`);
    ...
    if (collections.length > 0 && collections[0]) {
      console.log(`  lotl ls ${collections[0].name}`);
    }
    console.log(`  ${c.dim}# Get a document${c.reset}`);
    if (collections.length > 0 && collections[0]) {
      console.log(`  lotl get lotl://${collections[0].name}/path/to/file.md`);
    }
    console.log(`  ${c.dim}# Search within a collection${c.reset}`);
    if (collections.length > 0 && collections[0]) {
      console.log(`  lotl search "query" -c ${collections[0].name}`);
    }
  } else {
    console.log(`\n${c.dim}No collections. Run 'lotl collection add .' to index markdown files.${c.reset}`);
  }
  ...
  console.log(`  Model:       ${process.env.LOTL_TRANSFORMERS_MODEL ?? process.env.LOTL_EMBED_MODEL ?? DEFAULT_EMBED_MODEL_URI}`);
  console.log(`  Dtype:       ${process.env.LOTL_TRANSFORMERS_DTYPE ?? "q8"}`);
  if (process.env.LOTL_TRANSFORMERS_FILE) {
    console.log(`  File:        ${process.env.LOTL_TRANSFORMERS_FILE}`);
  }
  ...
  if (collectionsWithoutContext.length > 0) {
    ...
    tips.push(`  ${c.dim}lotl context add lotl://<name>/ "What this collection contains"${c.reset}`);
    tips.push(`  ${c.dim}lotl context add lotl://<name>/meeting-notes "Weekly team meeting notes"${c.reset}`);
  }
  ...
  if (collectionsWithoutUpdate.length > 0 && collections.length > 1) {
    ...
    tips.push(`  ${c.dim}lotl collection update-cmd <name> 'git stash && git pull --rebase --ff-only && git stash pop'${c.reset}`);
  }
...
async function updateCollections(): Promise<void> {
  ...
  if (collections.length === 0) {
    console.log(`${c.dim}No collections found. Run 'lotl collection add .' to index markdown files.${c.reset}`);
    closeDb();
    return;
  }
  ...
  if (needsEmbedding > 0) {
    console.log(`\nRun 'lotl embed' to update embeddings (${needsEmbedding} unique hashes need vectors)`);
  }
}
 ...
 */
// detectCollectionFromPath moved to cli/context-commands.ts

// contextAdd, contextList, contextRemove moved to cli/context-commands.ts
...
function getDocument(filename: string, fromLine?: number, maxLines?: number, lineNumbers?: boolean): void {
  ...
  // Handle virtual paths (lotl://collection/path)
    ...
    for (const name of names) {
      ...
      if (isVirtualPath(name)) {
        ...
        if (parsed) {
          ...
          doc = db.prepare(`
            SELECT
              'lotl://' || d.collection || '/' || d.path as virtual_path,
  ...
  for (const file of files) {
    ...
    if (file.bodyLength > maxBytes) {
      results.push({
        ...
        context,
        skipped: true,
        skipReason: `File too large (${Math.round(file.bodyLength / 1024)}KB > ${Math.round(maxBytes / 1024)}KB). Use 'lotl get ${file.displayPath}' to retrieve.`,
      });
      continue;
    }
...
function listFiles(pathArg?: string): void {
  ...
  if (!pathArg) {
    ...
    if (yamlCollections.length === 0) {
      console.log("No collections found. Run 'lotl collection add .' to index files.");
      closeDb();
      return;
    }
    ...
    for (const coll of collections) {
      console.log(`  ${c.dim}lotl://${c.reset}${c.cyan}${coll.name}/${c.reset}  ${c.dim}(${coll.file_count} files)${c.reset}`);
    }
    closeDb();
    return;
  }
  ...
  if (pathArg.startsWith('lotl://')) {
    // Virtual path format: lotl://collection/path
  ...
  if (!coll) {
    ...
    console.error(`Run 'lotl ls' to see available collections.`);
    closeDb();
    process.exit(1);
  }
  ...
  if (files.length === 0) {
    if (pathPrefix) {
      console.log(`No files found under lotl://${collectionName}/${pathPrefix}`);
    } else {
  ...
  for (const file of files) {
    ...
    // Dim the lotl:// prefix, highlight the filename
    console.log(`${sizeStr}  ${timeStr}  ${c.dim}lotl://${collectionName}/${c.reset}${c.cyan}${file.path}${c.reset}`);
  }
...
async function collectionAdd(pwd: string, globPattern: string, name?: string): Promise<void> {
  ...
  if (existingPwdGlob) {
    ...
    console.error(`  Name: ${existingPwdGlob.name} (lotl://${existingPwdGlob.name}/)`);
    console.error(`  Pattern: ${globPattern}`);
    console.error(`\nUse 'lotl update' to re-index it, or remove it first with 'lotl collection remove ${existingPwdGlob.name}'`);
    process.exit(1);
  }
...
async function indexFiles(pwd?: string, globPattern: string = DEFAULT_GLOB, collectionName?: string, suppressEmbedNotice: boolean = false, ignorePatterns?: string[]): Promise<void> {
  ...
  if (!collectionName) {
    throw new Error("Collection name is required. Collections must be defined in ~/.config/lotl/index.yml");
  }
...
function getEditorUriTemplate(): string {
  const envTemplate = process.env.LOTL_EDITOR_URI?.trim();
...
// showHelp + showVersion moved to cli/help-version.ts
...
const isMain = argv1 === __filename
  || argv1?.endsWith("/lotl.ts")
  || argv1?.endsWith("/lotl.js")
...
if (isMain) {
  ...
  if (cli.values.help && cli.command === "skill") {
    console.log("Usage: lotl skill <show|install> [options]");
    console.log("");
    console.log("Commands:");
    console.log("  show                 Print the packaged Lotl skill");
    console.log("  install              Install into ./.agents/skills/lotl");
    console.log("");
    console.log("Options:");
    console.log("  --global             Install into ~/.agents/skills/lotl");
    console.log("  --yes                Also create the .claude/skills/lotl symlink");
  ...
  switch (cli.command) {
    case "context": {
      ...
      if (!subcommand) {
        console.error("Usage: lotl context <add|list|rm>");
        console.error("");
        console.error("Commands:");
        console.error("  lotl context add [path] \"text\"  - Add context (defaults to current dir)");
        console.error("  lotl context add / \"text\"       - Add global context to all collections");
        console.error("  lotl context list                - List all contexts");
        console.error("  lotl context rm <path>           - Remove context");
        process.exit(1);
      }
      ...
      switch (subcommand) {
        case "add": {
          if (cli.args.length < 2) {
            console.error("Usage: lotl context add [path] \"text\"");
            console.error("");
            console.error("Examples:");
            console.error("  lotl context add \"Context for current directory\"");
            console.error("  lotl context add . \"Context for current directory\"");
            console.error("  lotl context add /subfolder \"Context for subfolder\"");
            console.error("  lotl context add / \"Global context for all collections\"");
            console.error("");
            console.error("  Using virtual paths:");
            console.error("  lotl context add lotl://journals/ \"Context for entire journals collection\"");
            console.error("  lotl context add lotl://journals/2024 \"Context for 2024 journals\"");
            process.exit(1);
          }
        ...
        case "remove": {
          if (cli.args.length < 2 || !cli.args[1]) {
            console.error("Usage: lotl context rm <path>");
            console.error("Examples:");
            console.error("  lotl context rm /");
            console.error("  lotl context rm lotl://journals/2024");
            process.exit(1);
          }
    ...
    case "get": {
      if (!cli.args[0]) {
        console.error("Usage: lotl get <filepath>[:line] [--from <line>] [-l <lines>] [--line-numbers]");
        process.exit(1);
      }
    ...
    case "multi-get": {
      if (!cli.args[0]) {
        console.error("Usage: lotl multi-get <pattern> [-l <lines>] [--max-bytes <bytes>] [--json|--csv|--md|--xml|--files]");
    ...
    case "collection": {
      ...
      switch (subcommand) {
        ...
        case "rm": {
          if (!cli.args[1]) {
            console.error("Usage: lotl collection remove <name>");
            console.error("  Use 'lotl collection list' to see available collections");
            process.exit(1);
          }
        ...
        case "mv": {
          if (!cli.args[1] || !cli.args[2]) {
            console.error("Usage: lotl collection rename <old-name> <new-name>");
            console.error("  Use 'lotl collection list' to see available collections");
            process.exit(1);
          }
        ...
        case "update-cmd": {
          ...
          if (!name) {
            console.error("Usage: lotl collection update-cmd <name> [command]");
        ...
        case "exclude": {
          const name = cli.args[1];
          if (!name) {
            console.error(`Usage: lotl collection ${subcommand} <name>`);
        ...
        case "info": {
          const name = cli.args[1];
          if (!name) {
            console.error("Usage: lotl collection show <name>");
            process.exit(1);
          }
        ...
        case "help":
        case undefined: {
          console.log("Usage: lotl collection <command> [options]");
          console.log("");
          console.log("Commands:");
          ...
          console.log("  lotl collection add ~/notes --name notes");
          console.log("  lotl collection update-cmd brain 'git pull'");
          console.log("  lotl collection exclude archive");
          process.exit(0);
        }

        default:
          console.error(`Unknown subcommand: ${subcommand}`);
          console.error("Run 'lotl collection help' for usage");
          process.exit(1);
      }
      break;
    }
    ...
    case "pull": {
      ...
      // @huggingface/transformers downloads on first use into ~/.cache/lotl/transformers/.
      console.log(`${c.bold}lotl pull is deprecated${c.reset}`);
      ...
      console.log(`Override via LOTL_TRANSFORMERS_MODEL / LOTL_TRANSFORMERS_DTYPE / LOTL_TRANSFORMERS_FILE.`);
      break;
    }

    case "search":
      if (!cli.query) {
        console.error("Usage: lotl search [options] <query>");
        process.exit(1);
      }
    ...
    case "vector-search": // undocumented alias
      if (!cli.query) {
        console.error("Usage: lotl vsearch [options] <query>");
        process.exit(1);
      }
    ...
    case "deep-search": // undocumented alias
      if (!cli.query) {
        console.error("Usage: lotl query [options] <query>");
        process.exit(1);
      }
    ...
    case "bench": {
      ...
      if (!fixturePath) {
        console.error("Usage: lotl bench <fixture.json> [--json] [-c collection]");
        console.error("");
        ...
        if (cli.values.daemon) {
          ...
          if (existsSync(pidPath)) {
            ...
            try {
              ...
              console.error(`Already running (PID ${existingPid}). Run 'lotl mcp stop' first.`);
              process.exit(1);
            } catch {
    ...
    case "skill": {
      ...
      switch (subcommand) {
        ...
        default:
          console.error(`Unknown subcommand: ${subcommand}`);
          console.error("Run 'lotl skill help' for usage");
          process.exit(1);
      }
      break;
    }
    ...
    case "memory": {
      ...
      if (subCmd === "store" || subCmd === "add") {
        ...
        if (!text) { console.error("Usage: lotl memory store <text>"); process.exit(1); }
        ...
        if (!query) { console.error("Usage: lotl memory recall <query>"); process.exit(1); }
        ...
        const id = cli.args[1];
        if (!id) { console.error("Usage: lotl memory forget <id>"); process.exit(1); }
        const result = memoryForget(db, id);
        console.log(result.deleted ? `Deleted: ${id}` : `Not found: ${id}`);
      } else if (subCmd === "extract") {
        const text = cli.args.slice(1).join(" ");
        if (!text) { console.error("Usage: lotl memory extract <text>"); process.exit(1); }
        ...
        if (!filePath) { console.error("Usage: lotl memory import <file.json>"); process.exit(1); }
        ...
        console.error("Usage: lotl memory <store|recall|forget|extract|stats|decay|import|export> [args]");
        process.exit(1);
      }
      closeDb();
      break;
    ...
    case "sync": {
      // Update all collections then embed pending documents
      try {
        await updateCollections();
      } catch (error) {
        console.error(error instanceof Error ? error.message : String(error));
    ...
    default:
      ...
      console.error("Run 'lotl --help' for usage.");
      process.exit(1);
  }
