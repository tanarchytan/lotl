 ...
 *
 * This module manages the YAML-based collection configuration at ~/.config/lotl/index.yml.
 ...
 * Config file will be ~/.config/lotl/{indexName}.yml
 */
...
function getConfigDir(): string {
  // Allow override via LOTL_CONFIG_DIR for testing
  if (process.env.LOTL_CONFIG_DIR) {
    return process.env.LOTL_CONFIG_DIR;
  }
  // Respect XDG Base Directory specification (consistent with store.ts)
  if (process.env.XDG_CONFIG_HOME) {
    return join(process.env.XDG_CONFIG_HOME, "lotl");
  }
  return join(homedir(), ".config", "lotl");
}
 ...
 * - File-based: reads from YAML file (default ~/.config/lotl/index.yml)
 ...
 * - File-based: writes to YAML file (default ~/.config/lotl/index.yml)
 */
