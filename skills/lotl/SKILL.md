---
name: lotl
description: Lotl — Living-off-the-Land memory for AI agents. BM25, vector, RRF fusion, LLM reranking, Weibull decay, temporal knowledge graph. Runs on SQLite + local ONNX — no new infra.
license: MIT
compatibility: Node.js >=22. Works standalone (CLI/MCP) or as OpenClaw plugin.
metadata:
  author: tanarchy
  version: "1.0.0"
allowed-tools: Bash(lotl:*), mcp__lotl__*
---

# Lotl — Search + Memory + Knowledge Graph
...
All provider config lives in `openclaw.json` under `plugins.entries.tanarchy-lotl.config`:
...
# LOTL_EMBED_DIMENSIONS=        # Optional, auto-detected
LOTL_RERANK_PROVIDER=zeroentropy
LOTL_RERANK_API_KEY=ze-your-key
LOTL_RERANK_MODEL=zerank-2
# LOTL_RERANK_MODE=rerank       # rerank (dedicated API) or llm (chat model)
